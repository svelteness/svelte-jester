...
describe('ESM transformer', () => {
  it('should transform with config in ESM format', async () => {
    ...
    const results = await runTransformer('BasicComp', { preprocess: svelteKitConfigPath })
  ...
  it('should accept compiler options', async () => {
    const code = await runTransformer('BasicComp', { compilerOptions: { css: false } })
  ...
  it('should pass and transform process.env.NODE_ENV variable', async () => {
    const code = await runTransformer('BasicComp', { preprocess: true, rootMode: 'upward' })
